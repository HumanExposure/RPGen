...
#' Downloads 2019 AHS Data and Formats for RPGen
#'
#' Selects columns and creates pool input (urban, location, 
#' housetyp, famcat, inccat) to create pool. Codes in lot and unit
#' size.
#'
#' Data from https://www.census.gov/programs-surveys/ahs/data/2019/ahs-2019-public-use-file--puf-.html
#'
#' @param   x A hyperlink to AHS 2019 Survey Data.
#' @return  Runtime for the complete download and test.
#' @export  RPGen_AHS.rda (where the underscore is a space)
...
AHS.download<-function(x){
  ...
  ahscols<-(toupper(c("acprimary","acsecndry","bathrooms","bedrooms","bld","control","cookfuel","cooktype",
                      "dishwash","division","dryer","elecamt","fridge","garage","gasamt","heatfuel","heattype",
                      ...
                      "unitfloors","unitsize","washer","watsource","weight","yrbuilt")))
  ...
  # Remove " ' " from dataframe (apostrophe) and changes to numeric (base R). AE
  ahs<- as.data.frame(sapply(ahs, function(x) gsub("\'", "", x)))
  ahs<- data.frame(apply(ahs, 2, as.numeric))
  ...
  ahs<-ahs[c("pool", "acprim", "acsec","rooms","baths","bedrms","built","hequip","heatfuel","sewdis","lot",
...
AHS.download("http://www2.census.gov/programs-surveys/ahs/2019/AHS%202019%20National%20PUF%20v1.1%20CSV.zip")
